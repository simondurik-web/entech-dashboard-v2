...
const pdfStyles = StyleSheet.create({
  ...
  recipient: { width: '11%' },
  destination: { width: '12%' },
  service: { width: '10%' },
  cost: { width: '6%', textAlign: 'right', paddingRight: 2 },
  residential: { width: '6%' },
  source: { width: '9%' },
  tracking: { width: '7%' },
  footer: {
...
function pdfRow(row: ShipmentRow) {
  return React.createElement(
    ...
    pdfCell(row.shipping_cost_usd === null ? '' : row.shipping_cost_usd.toFixed(2), pdfStyles.cost),
...
async function xlsxResponse(rows: ShipmentRow[], truncated: boolean, filename: string) {
  ...
  worksheet.columns = [
    ...
    { header: 'Shipping cost (USD)', key: 'shippingCost', width: 18 },
  ...
  for (const row of rows) {
    worksheet.addRow({
      ...
      service: row.service ?? '',
      // Number so Excel can SUM; blank (never 0) when the cost was not captured.
      shippingCost: row.shipping_cost_usd ?? '',
