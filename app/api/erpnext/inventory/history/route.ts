...
export async function GET(req: NextRequest) {
  ...
  // Committed events only (skip failed/in-flight rows so history shows what
  // actually happened). Oldest first so the UI can derive transitions.
  const { data: rows, error } = await supabaseAdmin
    .from('inventory_ops_log')
    .select('action, status, created_by, qty, warehouse, station_id, created_at')
    .eq('batch', batch)
