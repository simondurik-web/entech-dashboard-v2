...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (components) {
    await supabaseAdmin.from('bom_sub_assembly_components').delete().eq('sub_assembly_id', id)
    if (components.length) {
      const comps = components.map((c: Record<string, unknown>, i: number) => ({ ...c, sub_assembly_id: id, sort_order: i }))
