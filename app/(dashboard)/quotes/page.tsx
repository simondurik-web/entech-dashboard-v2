...
export default function QuotesPage() {
  ...
  const [columns, setColumns] = useState<ColumnDef<QuoteRecord>[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    fetch('/api/generic-sheet?gid=quotesRegistry')
      .then((res) => {
        if (!res.ok) throw new Error('Failed to fetch Quotes Registry data')
        return res.json()
      })
      .then(({ headers, data }: { headers: string[]; data: QuoteRecord[] }) => {
        // Build columns from headers
        const cols: ColumnDef<QuoteRecord>[] = headers.map((h) => ({
          key: h,
          label: h,
          sortable: true,
          filterable: true,
