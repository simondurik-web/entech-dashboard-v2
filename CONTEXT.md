...
2026-04-17: Shipped **Cost Change Log** tab on BOM Explorer (`app/(dashboard)/bom/page.tsx`). New 4th tab shows a unified timeline of cost + lead-time changes across individual items, sub-assemblies, and final assemblies — no more clicking into each item's history one at a time. New endpoint `GET /api/bom/cost-history` queries `bom_cost_history_with_details` with filters for date range, item type, and change type (cost vs lead time). Migration `20260417_bom_lead_time_tracking.sql` extends the individual-item trigger to also record `lead_time` changes into `bom_cost_history`. Rows are clickable → expand to show `affected_assemblies`. Uses top-level search box for part-number/description filtering. Spec: `specs/cost-change-log.md`. Branch: `claude/agitated-greider-8f10ca` — pending PR to staging.
