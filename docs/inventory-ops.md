...
| Move between bins | CARRIED: draft Stock Entry stamped `[op:key] [carried:so\|line\|qty]` FIRST (durable intent) → release (pinned SRE) → submit draft → re-reserve in the new bin on the SAME release line. Post-commit failure → `warning: reservation_transfer_failed`; EVERY move 200 is reservation-verified in the route before responding (resumed/replayed rows included) and restores strictly from the op's own stamped entry — never from history. Multi-pallet SREs, partially delivered SREs, and qty mismatches are refused in preflight. Bulk transfer SKIPS reserved pallets (reason `reserved`). Orphaned stamped DRAFTS (release failed with the reservation intact) are inert and reused by the retry. | move route erp() + post-op verify; `reservedMoveGuard` / `verifyOrRestoreMovedReservation` in inventory.ts |
