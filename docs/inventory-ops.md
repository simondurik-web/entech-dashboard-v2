...
| Move between bins | CARRIED: draft Stock Entry stamped `[op:key] [carried:so\|line\|qty]` FIRST (durable intent) → release (pinned SRE) → submit draft → re-reserve in the new bin on the SAME release line. Post-commit failure → `warning: reservation_transfer_failed`; EVERY move 200 is reservation-verified in the route before responding and restores strictly from the op's own stamped entry — never from history. Reconcile is READ-ONLY; a crash mid-op wedges 'pending' like every inventory op (admin unwedges to failed_pre_erp; the born checkpoint + armed draft make erp()'s re-run resume the carry). Unresolved checkpoints (`reservation:` in the op row's error) are swept by pallet FAMILY on the next fresh move, so a reloaded browser can't strand one. Multi-pallet / partially-delivered / line-less / qty- or bin-mismatched SREs are refused in preflight. Bulk transfer SKIPS reserved pallets (reason `reserved`). Stale stamped DRAFTS are DEFUSED (tags voided), never left armed. | move route erp() + post-op verify; `reservedMoveGuard` / `verifyOrRestoreMovedReservation` in inventory.ts |
