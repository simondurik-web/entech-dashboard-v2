...
| Move between bins | CARRIED: released (pinned SRE) → Material Transfer → re-reserved in the new bin on the SAME release line; failure → `warning: reservation_transfer_failed` + retry/reconcile restores from the cancelled-SRE trail (bounded by the op row's created_at). Multi-pallet SREs, partially delivered SREs, and qty mismatches are refused in preflight. Bulk transfer SKIPS reserved pallets (reason `reserved`). | move route erp() + reconcile; `reservedMoveGuard` / `verifyOrRestoreMovedReservation` in inventory.ts |
