    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="outline" size="sm">
          ...
          <span className="hidden sm:inline">{t('ui.customViews')}</span>
        </Button>
      </PopoverTrigger>
      <PopoverContent align="end" className="w-80 p-3 space-y-3">
        {userId ? (
          <div className="space-y-2">
            <p className="text-sm font-medium">{t('ui.saveCurrentView')}</p>
            <div className="flex items-center gap-2">
              <Input
                value={newName}
                onChange={(e) => setNewName(e.target.value)}
                placeholder={t('ui.viewNamePlaceholder')}
                className="h-8"
                onKeyDown={(e) => { if (e.key === 'Enter') saveView() }}
              />
              <Button size="sm" onClick={saveView} disabled={saving || !newName.trim()}>
                {saved ? <Check className="size-3.5" /> : t('ui.save')}
              </Button>
            </div>
            ...
            {saved && <p className="text-xs text-green-500">{t('ui.savedFindInReports')}</p>}
          </div>
        ) : (
          <p className="text-sm text-muted-foreground">{t('ui.signInToSaveViews')}</p>
        )}

        {views.length > 0 && (
          <div className="space-y-1 max-h-52 overflow-y-auto border-t pt-2">
            <p className="text-xs font-medium text-muted-foreground mb-1">{t('ui.quickApply')}</p>
            {views.map((v) => (
              <button
                key={v.id}
              ...
              >
                ...
                <span className="text-[11px] text-muted-foreground ml-2">{t('ui.by')} {v.user_id}</span>
              </button>
            ))}
          </div>
        )}
